...
export const getUsersLikedMovies = createAsyncThunk(
  ...
  async (email) => {
    const {
      data: { movies },
    } = await axios.get(`https://netflix-clone-backend-smoky.vercel.app/api/user/liked/${email}`);
    return movies;
  }
);
...
export const removeMovieFromLiked = createAsyncThunk(
  ...
  async ({ movieId, email }) => {
    const {
      data: { movies },
    } = await axios.put("https://netflix-clone-backend-smoky.vercel.app/api/user/remove", {
      email,
      movieId,
    });
    return movies;
